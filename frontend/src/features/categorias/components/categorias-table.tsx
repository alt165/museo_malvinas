...
export function CategoriasTable({ canEdit, categorias, isDeleting = false, onDelete }: CategoriasTableProps) {
  const columns = useMemo<ColumnDef<CategoriaObjetoResponseDTO>[]>(
    () => [
      {
        accessorKey: "nombre",
        header: "Nombre",
      ...
      },
      {
        id: "activo",
        header: "Activo",
        cell: ({ row }) => (row.original.activo === false ? "No" : "Si")
      },
      {
        id: "acciones",
        header: "Acciones",
        cell: ({ row }) => (
          <div className="flex justify-end gap-2">
            <Link className="inline-flex h-8 items-center gap-1 rounded-md border px-2 text-xs hover:bg-muted" href={`/categorias/${row.original.id}`}>
              <Search className="h-3.5 w-3.5" />
              Ver
            </Link>
            {canEdit ? (
              <>
                <Link className="inline-flex h-8 items-center gap-1 rounded-md border px-2 text-xs hover:bg-muted" href={`/categorias/${row.original.id}/editar`}>
