...
export function VeteranoForm({ initialValue, isSubmitting, onSubmit, submitLabel }: VeteranoFormProps) {
  const { formState: { errors }, handleSubmit, register } = useForm<VeteranoFormValues>({
  ...
  return (
    <form className="max-w-4xl space-y-5 rounded-lg border p-5" onSubmit={handleSubmit((values) => onSubmit({
      ...
      <Field label="Historia" error={errors.historia?.message}><textarea className="min-h-32 w-full rounded-md border bg-background px-3 py-2 text-sm" {...register("historia")} /></Field>
